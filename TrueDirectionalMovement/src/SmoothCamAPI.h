...
{
	constexpr const char* SmoothCamPluginName = "SmoothCam";

#ifdef SMOOTHCAM_API_COMMONLIB
	using PluginHandle = size_t;
	using Actor = ConsoleRE::Actor;
	using TESObjectREFR = ConsoleRE::TESObjectREFR;
	using NiCamera = ConsoleRE::NiCamera;
	using NiPoint3 = ConsoleRE::NiPoint3;
#endif
	...
	/// <returns></returns>
	[[nodiscard]] inline bool RegisterInterfaceLoaderCallback(SKSEMessagingInterface * skseMessaging,
		PluginHandle myPluginHandle, InterfaceLoaderCallback && callback) noexcept
	{
		...
		return skseMessaging->RegisterListener(
			...
			[](SKSEMessagingInterface::Message* msg) {
				if (msg->sender && strcmp(msg->sender, SmoothCamPluginName) != 0)
					return;
				if (msg->type != 0)
					return;
				if (msg->dataLen != sizeof(PluginResponse))
					return;
	...
	/// <param name="version">The interface version to request</param>
	/// <returns>If any plugin was listening for this request, true. See skse/PluginAPI.h</returns>
	[[nodiscard]] inline bool RequestInterface(const Interface::MessagingInterface* skseMessaging,
		InterfaceVersion version = InterfaceVersion::V3) noexcept
	{
		InterfaceRequest req = {};
		req.interfaceVersion = version;
		...
		return skseMessaging->DispatchLiseners(
			0,
			&cmd, 
			sizeof(PluginCommand),
			SmoothCamPluginName
		);
	}
	...
	/// <param name="callback">A callback function receiving both the interface pointer and interface version</param>
	/// <returns></returns>
	[[nodiscard]] inline bool RegisterInterfaceLoaderCallback(const Interface::MessagingInterface* skseMessaging,
		InterfaceLoaderCallback&& callback) noexcept
	{
		static InterfaceLoaderCallback storedCallback = callback;

		return skseMessaging->RegisterPluginCallback
		(
			SmoothCamPluginName,
			[](Interface::MessagingInterface::Message* msg) 
			{
				...
				if (msg->dataSize != sizeof(PluginResponse))
					return;
