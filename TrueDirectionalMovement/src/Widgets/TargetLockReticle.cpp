...
namespace Scaleform
{
	...
	void TargetLockReticle::ChangeTarget(ConsoleRE::ObjectRefHandle a_refHandle, ConsoleRE::NiPointer<ConsoleRE::NiAVObject> a_targetPoint)
	{
		AddWidgetTask([=]() 
		{
	...
	void TargetLockReticle::UpdatePosition()
	{
		...
		{
			ConsoleRE::NiPoint3 targetWorldPos = _targetPoint ? _targetPoint->world.translate : _refHandle.get()->GetLookingAtLocation();
			...
			ConsoleRE::NiCamera::WorldPtToScreenPt3((float(*)[4])g_worldToCamMatrix, *g_viewPort, targetWorldPos, _desiredScreenPos.x, _desiredScreenPos.y, _depth, 1e-5f);
	...
	void TargetLockReticle::UpdateInfo()
	{
		...
		auto actor = _refHandle.get()->As<ConsoleRE::Actor>();
		if (!actor) 
		{
			_widgetState = WidgetState::kRemoved;
			return;
	...
	void TargetLockReticle::StartInterpolation(InterpMode a_interpMode)
	{
		...
		//	ConsoleRE::GRectF rect = view->GetVisibleFrameRect();
		//	_lastScreenPos.x = rect.right * 0.5f;
		//	_lastScreenPos.y = rect.bottom * 0.5f;
		//} else {
		//	ConsoleRE::GFxValue::DisplayInfo displayInfo;
