...
bool GetAngle(ConsoleRE::TESObjectREFR* a_target, AngleZX& angle)
{
	...
	ConsoleRE::NiPoint3 targetPos = a_target->GetPosition();
...
ConsoleRE::NiPoint3 GetCameraPos()
{
	...
	if (playerCamera->currentState == playerCamera->cameraStates[ConsoleRE::CameraStates::kFirstPerson] || 
		playerCamera->currentState == playerCamera->cameraStates[ConsoleRE::CameraStates::kThirdPerson] ||
		playerCamera->currentState == playerCamera->cameraStates[ConsoleRE::CameraStates::kMount]) {
		ConsoleRE::NiNode* root = playerCamera->cameraRoot.get();
		if (root) {
		...
		ConsoleRE::NiPoint3 playerPos = player->GetLookingAtLocation();
...
bool GetTorsoPos(ConsoleRE::Actor* a_actor, ConsoleRE::NiPoint3& point)
{
	...
	ConsoleRE::TESRace* race = a_actor->GetRace();
	if (!race) {
		return false;
	}

	ConsoleRE::NiAVObject* object = a_actor->Get3D2();
	if (!object) {
		return false;
	}
	...
	ConsoleRE::BGSBodyPart* bodyPart = bodyPartData->parts[ConsoleRE::BGSBodyPartDefs::LIMB_ENUM::kTorso];
	if (!bodyPart) {
		return false;
	}	
...
bool GetTargetPointPosition(ConsoleRE::ObjectRefHandle a_target, const char* a_targetPoint, ConsoleRE::NiPoint3& a_outPos)
{
	...
	ConsoleRE::BSFixedString targetPointName = a_targetPoint;
...
bool PredictAimProjectile(ConsoleRE::NiPoint3 a_projectilePos, ConsoleRE::NiPoint3 a_targetPosition, ConsoleRE::NiPoint3 a_targetVelocity, float a_gravity, ConsoleRE::NiPoint3& a_projectileVelocity)
{
	...
	ConsoleRE::NiPoint3 targetToProjectile = a_projectilePos - a_targetPosition;
	float distanceSquared = targetToProjectile.SqrLength();
	float distance = std::sqrtf(distanceSquared);
	ConsoleRE::NiPoint3 direction = targetToProjectile;
	direction.Unitize();
	ConsoleRE::NiPoint3 targetVelocityDirection = a_targetVelocity;
